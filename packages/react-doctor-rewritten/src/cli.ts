...
const resolveCliScanOptions = (
  ...
  return {
    ...
    noBranding,
    noBrandingThread: flags.hideBrandingPr
      ? "pr"
      : flags.hideBranding
        ? "full"
        : (userConfig?.noBrandingThread ?? "full"),
  };
};
...
const resolveEffectiveDiff = (
  ...
  // --hide-branding-pr forces diff mode against the default base branch
  // (so the PR-scoped HTML report only covers files changed on this branch).
  if (flags.hideBrandingPr) return true;
...
const validateModeFlags = (flags: CliFlags): void => {
  ...
  if (flags.hideBranding && flags.hideBrandingPr) {
    throw new Error("Cannot combine --hide-branding and --hide-branding-pr; pick one.");
  }
  const noBranding = flags.hideBranding || flags.hideBrandingPr;
  if (noBranding && (flags.json || flags.score || flags.annotations)) {
    throw new Error(
      "--hide-branding / --hide-branding-pr cannot be combined with --json, --score, or --annotations.",
    );
  }
};
...
const program = new Command()
  ...
  .action(async (directory: string, flags: CliFlags) => {
    ...
    const isNoBrandingMode = flags.hideBranding || flags.hideBrandingPr;
    const isQuiet = isScoreOnly || isJsonMode || isNoBrandingMode;
    ...
    if (isJsonMode || isNoBrandingMode) {
...
// (browser commands stripped from this fork — playwright + react-doctor-browser
// dependency was a heavy add for a feature unrelated to the CI/HTML-report use
// case this fork targets. Use the upstream `react-doctor` package if you need
// `react-doctor browser ...`.)

// HACK: when stdout is piped into a process that closes early, Node throws an
