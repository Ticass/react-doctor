...
const mergeScanOptions = (
  ...
  noBrandingThread: inputOptions.noBrandingThread ?? userConfig?.noBrandingThread ?? "full",
});
...
const runScan = async (
  ...
  // HACK: --hide-branding / --hide-branding-pr: emit clean GitHub-flavored
  // HTML instead of the ASCII-art terminal report, then return early so the
  // normal rendering path is completely skipped.
  if (options.noBranding) {
    const previouslySilent = isLoggerSilent();
    setLoggerSilent(false);
    process.stdout.write(
      `${buildNoBrandingReport(diagnostics, scoreResult, options.noBrandingThread)}\n`,
    );
