...
`,
  ...
  it("does NOT flag state read via a two-level nullish-coalescing derived constant (colleague false positive)", async () => {
    // Reported as false positive: state was flagged even though it flows into
    // JSX through a derived constant:
    //   const selectedException = updatedException ?? exception;
    //   const associatedIds = updatedShipmentReferenceIds ?? selectedException.attributes.shipmentIds ?? [];
    //   return <Table data={associatedIds} />;  // ← associatedIds IS in the return
    const projectDir = setupReactProject(tempRoot, "issue-fp-nullish-chain", {
      files: {
        "src/exception-drawer.tsx": `import { useState } from "react";

declare const Table: (props: { data: string[] }) => null;

export const ExceptionDetailDrawer = ({
  exception,
}: {
  exception: { attributes: { shipmentIds: string[] } };
}) => {
  const [updatedException, setUpdatedException] = useState<typeof exception | null>(null);
  const [updatedShipmentReferenceIds, setUpdatedShipmentReferenceIds] = useState<string[] | null>(null);

  const handleSave = (ids: string[]) => {
    setUpdatedShipmentReferenceIds(ids);
  };

  const handleExceptionUpdate = (e: typeof exception) => {
    setUpdatedException(e);
  };

  const selectedException = updatedException ?? exception;
  const associatedShipmentReferenceIds =
    updatedShipmentReferenceIds ?? selectedException.attributes.shipmentIds ?? [];

  return (
    <div>
      <Table data={associatedShipmentReferenceIds} />
      <button onClick={() => handleSave(["a"])}>Save</button>
      <button onClick={() => handleExceptionUpdate(exception)}>Update</button>
    </div>
  );
};
`,
    ...
    expect(
      findStateOnlyInHandlersDiagnostics(diagnostics, "src/exception-drawer.tsx"),
    ).toHaveLength(0);
  });
